...
local function get_unique_prefix(filename)
    ...
    if #other_filenames == 0 then return fn.fnamemodify(filename, ':t') end

    filename = reverse(filename)
    other_filenames = map(reverse, other_filenames)

    local index = math.max(unpack(map(function(f)
        for i = 1, #f do
            if filename:sub(i, i) ~= f:sub(i, i) then return i end
        end
        return 1
    end, other_filenames)))
...
function Buffer:new(b, index)
    ...
    if b.name then buffer.path = b.name end
    ...
    buffer.unique_prefix = #fn.getbufinfo({
        buflisted = 1
    }) ~= 1 and get_unique_prefix(buffer.path) or ''

    if has_devicons then
        local name = buffer.type == 'terminal' and 'terminal' or
                         fn.fnamemodify(buffer.path, ':t')
